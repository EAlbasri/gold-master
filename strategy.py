...
def detect_breakout_continuation(symbol, direction, regime):
    ...
    width = max(range_high - range_low, 6.0)
    body = candle_body(m15, -1)
    mean_body = avg_body(m15, 12)

    if direction == "buy":
        if float(last["close"]) > range_high and body > mean_body * 1.25:
            return _make_candidate(
                "breakout_continuation", "buy", float(last["close"]), min(float(last["low"]), range_high), float(last["close"]) + width,
                "Strong M15 upside displacement through recent range high.", regime,
            )
    else:
        if float(last["close"]) < range_low and body > mean_body * 1.25:
            return _make_candidate(
                "breakout_continuation", "sell", float(last["close"]), max(float(last["high"]), range_low), float(last["close"]) - width,
                "Strong M15 downside displacement through recent range low.", regime,
            )
    return None
...
def detect_breakout_retest(symbol, direction, regime):
    ...
    if direction == "buy":
        ...
        if breakout and retest:
            return _make_candidate(
                "breakout_retest", "buy", float(last["close"]), min(float(last["low"]), range_high), float(last["close"]) + width,
                "M15 breakout held and retested successfully from above.", regime,
            )
    else:
        ...
        if breakout and retest:
            return _make_candidate(
                "breakout_retest", "sell", float(last["close"]), max(float(last["high"]), range_low), float(last["close"]) - width,
                "M15 breakdown held and retested successfully from below.", regime,
            )
    return None
...
def detect_trend_pullback(symbol, direction, regime):
    ...
    if direction == "buy":
        ...
        if trend_ok and touched and confirm:
            ...
            return _make_candidate(
                "trend_pullback", "buy", float(last["close"]), invalidation, max(target, float(last["close"]) + 10.0),
                "Bull trend pullback into M15 value zone with continuation confirmation.", regime,
            )
    else:
        trend_ok = float(m15["ema20"].iloc[-1]) < float(m15["ema50"].iloc[-1])
        ...
        if trend_ok and touched and confirm:
            ...
            return _make_candidate(
                "trend_pullback", "sell", float(last["close"]), invalidation, min(target, float(last["close"]) - 10.0),
                "Bear trend pullback into M15 value zone with continuation confirmation.", regime,
            )
    return None
...
def detect_liquidity_reversal(symbol, direction, regime):
    ...
    return _make_candidate(
        "liquidity_reversal", direction, float(sweep["close_after_sweep"]), invalidation, target,
        "Liquidity sweep reclaimed with VWAP and reversal confirmation.", regime,
    )
...
def detect_impulse_continuation(symbol, direction, regime):
    ...
    if direction == "buy":
        ...
        if shallow and confirm:
            return _make_candidate(
                "impulse_continuation", "buy", float(last["close"]), retrace_low,
                impulse_high + max(impulse_high - impulse_low, 10.0),
                "Strong upside impulse followed by shallow retracement and hold.", regime,
            )
    else:
        ...
        if shallow and confirm:
            return _make_candidate(
                "impulse_continuation", "sell", float(last["close"]), retrace_high,
                impulse_low - max(impulse_high - impulse_low, 10.0),
                "Strong downside impulse followed by shallow retracement and hold.", regime,
            )
    return None
...
def generate_setup_candidates(symbol):
    ...
    for direction in directions:
        ...
        for detector in detectors:
            ...
            if candidate:
                planned = build_trade_plan(candidate, snapshot)
